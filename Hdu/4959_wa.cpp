...
struct dot {
	...
	dot operator - ( dot d ) {return dot ( a-d.a,b-d.b );}
	dot operator * ( dot d ) {return dot ( a*d.a+5*b%MOD*d.b,b*d.a+a*d.b );}
	dot operator * ( LL i ) {return dot ( f(i)*a,f(i)*b );}
	dot operator / ( dot x ) {
		LL c=x.a,d=x.b;
...
//	freopen("wa.txt","w",stdout);
	...
	while(T--){
		...
		for(j=0;j<=k;j++){
			dot qj=(phi^j)*(_phi^(k-j)),tmp;
			if(qj.a==1&&qj.b==0)tmp=dot(n+1,0);
			else tmp=((qj^(n+1))-ONE)/(qj-ONE)/qj;
			dot mul=per*((phi+dot(2,0))^j)*((dot(-2,0)-_phi)^(k-j))*comb[j];
			ret=ret+tmp*mul;
		}
		cout<<f(ret.a)<<endl;
	}
