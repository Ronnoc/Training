...
|2014-8-16|poj1742|dp|_|2A|二进制多重背包or单调队列:dp[j]=max{j-k*A[i]@0<=k<=C[i]}|卡常,二进制WAonC++单调TLEonG++|
|2014-8-16|poj1821|dp|_|5S|单调队列优化max{dp[k]-P[i]*k@j-L[i]<=k<j,S[i]<=j}斜率优化{X=i,Y=D[i],下凸包,P[i]}:下凸包点首点不是最优解仅当最小斜率不小于决策斜率.}|维护下凸包时由于范围限制去点需要还原被删掉的点;下标写错|
___
|**Abbr.**|**Full**|
|:---:|:---:|
