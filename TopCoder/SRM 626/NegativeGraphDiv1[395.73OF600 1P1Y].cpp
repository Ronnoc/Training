...
LL dp[2][55][55];
vector<PII>G[55];
void play( LL C[][55],LL A[][55],LL B[][55],int N ) {
	LL D[55][55];
	int i,j,k;
	for ( i=1; i<=N; i++ )for ( j=1; j<=N; j++ )D[i][j]=min( A[i][j],B[i][j] );
	for ( i=1; i<=N; i++ )for ( j=1; j<=N; j++ )for ( k=1; k<=N; k++ )
				cmin( D[i][j],A[i][k]+B[k][j] );
	memcpy( C,D,sizeof D );
}
class NegativeGraphDiv1 {
public:
	long long findMin( int N, vector <int> U, vector <int> V, vector <int> W, int M ) {
		long long ret=0;
		int i,j,k;
		for ( i=1; i<=N; i++ )G[i].clear();
		for ( i=0; i<U.SZ; i++ )G[U[i]].PB( MP( V[i],W[i] ) );
		for ( i=1; i<=N; i++ )for ( j=1; j<=N; j++ )for ( k=0; k<2; k++ )
					dp[k][i][j]=1LL<<61;
		for ( i=1; i<=N; i++ ) {
			queue<PII>L;
			L.push( MP( i,0 ) );
			dp[0][i][i]=0;
			while ( !L.empty() ) {
				int u=L.front().AA,k=L.front().BB;
				L.pop();
				for ( j=0; j<G[u].SZ; j++ ) {
					int v=G[u][j].AA;
					int d=G[u][j].BB;
					if ( dp[k][i][v]>dp[k][i][u]+d ) {
						dp[k][i][v]=dp[k][i][u]+d;
						L.push( MP( v,k ) );
					}
					if ( !k&&dp[k+1][i][v]>dp[k][i][u]-d ) {
						dp[k+1][i][v]=dp[k][i][u]-d;
						L.push( MP( v,k+1 ) );
					}
				}
			}
		...
		while ( M ) {
			if ( M&1 )play( dp[0],dp[0],dp[1],N );
			play( dp[1],dp[1],dp[1],N );
			M/=2;
		}
		return dp[0][1][N];
	}
...
int run_test_case( int casenum ) {
	switch ( casenum ) {
		...
		case 1: {
				...
				int weight[]              = {100};
				int charges               = 100000;
				long long expected__      = -10000000LL;
