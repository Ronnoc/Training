...
public:
  double getAverage (vector <string> rat) {
    ...
    string SIN="";
    int i,j;
    for(i=0;i<rat.SZ;i++)SIN+=rat[i];
    istringstream sin(SIN);
    vector<int>L;
    int x,Elly;
    while(sin>>x){
			if(L.empty())Elly=x;
    ...
    vector<int>rm[3000];
    for(id=1,i=0;i<L.SZ;i+=R,id++)
			for(j=i;j<L.SZ&&j<i+R;j++)rm[id].PB(L[j]);
		id--;
    int w=-1;
    for(i=1;i<=id;i++)
			for(j=0;j<rm[i].SZ;j++)
...
//    cout<<"~~~~"<<w<<endl;
    ...
    RM.PB(Elly);
		for(i=1;i<id;i++){
			if(i==w)continue;
			double avg=0;
			for(j=0;j<rm[i].SZ;j++)avg+=rm[i][j];
			avg/=rm[i].SZ;
			RM.PB(avg);
		}
		if(id==w){
			double ans=0;
			for(i=0;i<RM.SZ;i++)ans+=RM[i];
			ans/=RM.SZ;
			return ans;
		}
		double sum=0;
		for(i=0;i<RM.SZ;i++)sum+=RM[i];
		cout<<sum/RM.SZ<<endl;
		for(i=0;i<rm[id].SZ;i++){
			sum+=rm[id][i];
			ret+=(sum/(RM.SZ+1));
			sum-=rm[id][i];
		}
		ret+=(R-rm[id].SZ)*sum/RM.SZ;
    return ret/R;
  }
};
